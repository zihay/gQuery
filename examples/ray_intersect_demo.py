...
class RayIntersectionVisualizer(PolylineViewer):
    def add_ray_intersection_visualization(
        ...
        n = Array2(0.0, 0.0)  # Default normal for ray origin
        on_boundary = Bool(False)  # Not starting on boundary
        intersection_record = self.polyline.intersect(
            query_origin, query_direction, n, on_boundary, Float(r_max))
        ...
        def ray_intersection_gui_callback():
            ...
            if opened:
                ...
                if changed_any:
                    ...
                    intersection_record = dr.zeros(Intersection)
                    if current_use_bvh:
